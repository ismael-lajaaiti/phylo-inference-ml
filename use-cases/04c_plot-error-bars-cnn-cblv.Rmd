---
title: "Plot Error Bars CNN CBLV"
...
```{r}
param <- readRDS("data/predictions-bisse-cblv.rds")
...
error.list <- fillErrorList(
  error.list, true.param,
...
addBarName <- function(df, xpos, inout, space = .4) {
  ...
  for (i in 1:n) {
    ...
    if (
      names[i] == "CNN-CBLV-notips") {
      name <- TeX("CNN-$CBLV^{notips}$")
      y <- y + .05 * as.integer(inout[i] == "in")
    }
    if (inout[i] == "in") {
      ...
      legend(
        x = xpos[i], y, name, box.col = "white", bg = "white", cex = 1.,
...
# Plot.
pdf("figures/fig-error-bars-cnn-cblv.pdf", width = 6 * 1.62, height = 3)
