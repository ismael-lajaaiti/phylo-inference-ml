---
title: "Plot Error Bars Main"
output: html_document
editor_options: 
  chunk_output_type: console
---

Set up: source main file set seed.
...
names(pred.param.crbd) <- c(
  "MLE", "CNN-CBLV", "RNN-LTT", "CNN-LTT", "DNN-SS",
  "GNN-PHY"
)
...
names(pred.param.bisse) <- c(
  "MLE", "RNN-LTT", "CNN-LTT", "CNN-CBLV", "DNN-SS",
  "GNN-PHY"
)
```


Specify parameter and prediction method names.
...
Set inner ranges inside which we consider the predicted values.
...
# Fill.
error.list.crbd <- fillErrorList(
  error.list.crbd, true.param.crbd,
  pred.param.crbd, name.param.crbd,
  name.method.crbd, range.in.crbd
)

error.list.bisse <- fillErrorList(
  error.list.bisse, true.param.bisse,
...
n_error <- length(name.method.crbd)
...
Sort dataframes by decreasing error.
...
plotBar <- function(df, name.param, ylab = "", legend = FALSE, mle_line = FALSE,
  ...
  if (mle_line) {
    abline(v = df["SSD", "MLE"], col = "black", lty = "dashed")
  }
...
addBarName <- function(df, xpos, inout, space = .4) {
  ...
  for (i in 1:n) {
    ...
    if (inout[i] == "in") {
      ...
      legend(
        x = xpos[i], y, names[i], box.col = "white", bg = "white", cex = 1.4,
