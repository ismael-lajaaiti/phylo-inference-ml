...
n_trees <- 10200 # total number of trees (train + test)
...
net1 <- nn_module(
    ...
    initialize = function(){
      ...
      self$fc3 <- nn_linear(in_features = 78, out_features = 78)
      self$fc4 <- nn_linear(in_features = 78, out_features = 2)
    }, 
...
```{r}
plot_loss_records(net1.fit)
```

Get the predictions 

```{r}
preds <- predict(net1.fit, test_dl)
...
```{r}
plot_loss_records(net2.fit)
```

Get the predictions.
```{r}
preds <- predict(net2.fit, test_dl2)
