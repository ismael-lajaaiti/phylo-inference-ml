...
create_nodes_data_frame <- function(tree) {
    ...
    df.nodes <- data.frame(
        index = rep(NA, n), # index of the node
...
create_edges_data_frame <- function(tree) {
    ...
    df.edges <- data.frame(
        index = rep(NA, n), # index of the edge
...
get_bl_ss <- function(df.edges) {
    ss.bl <- list(
        "a.mean" = -999, "a.med" = -999, "a.var" = -999,
...
get_topo_ss <- function(df.nodes, tree) {
    ss.topo <- list(
        "height" = -999, "colless" = -999, "sackin" = -999,
...
create_ss.names <- function() {
    ...
    ss.names <- c(
        ss.names, ltt.names.topo, ltt.names.slope,
        ltt.names.coord
    )
...
convert_ss_dataframe_to_dataset <- function(df) {
    ss_dataset <- torch::dataset(
        ...
        initialize = function(df, name.param, exclude.param = "") {
            ...
            # input data
            topo.names <- c(
                "height", "colless", "sackin", "wdratio", "deltaw", "max_ladder",
                "il_nodes", "stair1", "stair2"
            )
